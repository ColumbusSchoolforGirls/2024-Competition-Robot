...
public class DriveTrain {
  ...
  private final AHRS gyro = new AHRS(NavXComType.kMXP_SPI);
    ...
    public SparkMax frontLeft = new SparkMax(Constants.FRONT_LEFT_ID, MotorType.kBrushless); 
    //fake id -- need to change when we have a real chassi
    public SparkMax backLeft = new SparkMax(Constants.BACK_LEFT_ID, MotorType.kBrushless);
    //fake id -- need to change when we have a real chassi
    public SparkMax frontRight = new SparkMax(Constants.FRONT_RIGHT_ID, MotorType.kBrushless);
    //fake id -- need to change when we have a real chassi
    public SparkMax backRight = new SparkMax(Constants.BACK_RIGHT_ID, MotorType.kBrushless);
    //fake id -- need to change when we have a real chassi
    ...
    void setBrakeMode() {
        ...
        // We don't need it to brake anymore, keep it at coast
        config.idleMode(SparkBaseConfig.IdleMode.kCoast);
        frontLeft.configure(config, SparkBase.ResetMode.kNoResetSafeParameters, SparkBase.PersistMode.kNoPersistParameters);
        backLeft.configure(config, SparkBase.ResetMode.kNoResetSafeParameters, SparkBase.PersistMode.kNoPersistParameters);
        frontRight.configure(config, SparkBase.ResetMode.kNoResetSafeParameters, SparkBase.PersistMode.kNoPersistParameters);
        backRight.configure(config, SparkBase.ResetMode.kNoResetSafeParameters, SparkBase.PersistMode.kNoPersistParameters);
    ...
    public void setTeleop() {
        if (driveController.getXButtonPressed()) {
            if (frontLeft.configAccessor.getIdleMode() == IdleMode.kCoast) {
                setBrakeMode();
            } else {
            setCoastMode();
            }
